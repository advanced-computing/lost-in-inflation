...
@pytest.fixture
def pce_df():
    return load_csv_data("MoM PCE.csv", drop_cols=['Year', 'Month'], create_date_from=['Year', 'Month'])

@pytest.fixture
def china_mxp_df():
    return load_csv_data("EIUCOCHNTOT.csv", period_col="Period", drop_cols=['Year', 'Period'], start_date="2018-01-01")

@pytest.fixture
def inflation_df():
    return load_csv_data("monthly-inflation-data.csv", index_col="Label", date_col="Label")
